...
namespace ncore
{
    ...
    log_info_t&       g_GetLogInfo_ST() { return g_ST_LogInfo; }

    static log_info_t::_delegate* sGetLogInfoFunc = g_GetLogInfo_ST;

    void g_SetGetLogInfo(log_info_t::_delegate* Function) { sGetLogInfoFunc = Function; }

    log_to_console_imp sLogDefaultToConsole;
    logger_t*          log_t::sLogger[log_t::COUNT] = {&sLogDefaultToConsole, &sLogDefaultToConsole, &sLogDefaultToConsole, &sLogDefaultToConsole};
    ...
    //
    //-------------------------------------------------------------------------------------------------
    void log_t::write(elevel inLevel, bool _value) { sLogger[inLevel]->write(_value); }

    void log_t::write(elevel inLevel, f64 _value) { sLogger[inLevel]->write(_value); }

    void log_t::write(elevel inLevel, s32 _value) { sLogger[inLevel]->write(_value); }

    void log_t::write(elevel inLevel, s64 _value) { sLogger[inLevel]->write(_value); }
    ...
    void log_t::writeLine(elevel inLevel, s32 _value) { sLogger[inLevel]->writeLine(_value); }

    void log_t::writeLine(elevel inLevel, s64 _value) { sLogger[inLevel]->writeLine(_value); }
    ...
    void __LogMessage2(const char* channel, const char* msgFormat, const va_t* argv, s32 argc)
    {
        ...
        crunes_t logformatrunes = make_crunes((ascii::pcrune)logformat, (ascii::pcrune)logformat + logstrlen);
        crunes_t msgformatrunes = make_crunes((ascii::pcrune)msgFormat, (ascii::pcrune)msgFormat + msgstrlen);
    ...
    void __LogFlush(void) { log_t::flush(); }
