    ...
    void crunes_t::skip(u32& cursor, const crunes_t& skip_chars) const
    {
        switch (get_type())
        {
            ...
            case utf8::TYPE:
                ...
                {
                    u32     next = cursor;
                    uchar32 c    = utf::read(m_utf8,  next);
                    if (!skip_chars.contains(c))
                        break;
                    cursor = next;
                }
            ...
            case utf16::TYPE:
                ...
                {
                    u32     next = cursor;
                    uchar32 c    = utf::read(m_utf16, next);
                    if (!skip_chars.contains(c))
                        break;
                    cursor = next;
                }
            ...
            case utf32::TYPE:
                ...
                {
                    u32     next = cursor;
                    uchar32 c    = utf::read(m_utf32, next);
                    if (!skip_chars.contains(c))
                        break;
                    cursor = next;
                }
            ...
            switch (m_runes.get_type())
            {
                ...
                case utf8::TYPE:
                    while (m_cursor < m_runes.m_end)
                    {
                        u32 next = m_cursor;
                        skipped += 1;
                        c = utf::read(m_runes.m_utf8,  next);
						if (!s_contains_char(chars, num_chars, c))
                            break;
                        m_cursor = next;
					}
                ...
                case utf16::TYPE: 
                    while (m_cursor < m_runes.m_end)
                    {
                        u32 next = m_cursor;
                        skipped += 1;
                        c = utf::read(m_runes.m_utf16,  next);
                        if (!s_contains_char(chars, num_chars, c))
                            break;
                        m_cursor = next;
                    }
