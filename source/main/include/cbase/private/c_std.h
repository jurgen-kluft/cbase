...
#undef TMTT1
		...
		struct dtorsr
		{
			inline void operator()(U first, U last) { dtors (first, last); }
		};

		template<typename U>
		struct dtorsr<U, true>
		{
		...
		inline void				destroy (ForwardIterator first, ForwardIterator last)
		{
			...
			dtorsr<ForwardIterator, cstd::typetrait_t<valuetype>::isSystem>()(first, last);
		}
		...
		{
			typedef typename ForwardIterator::value_type	valuetype;
			if (cstd::typetrait_t<valuetype>::isSystem)
			{
				//
			}
