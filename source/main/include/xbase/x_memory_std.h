...
namespace xcore
{
	...
	{
	public:
		static void			memset(void* inDest, u32 inValue, u32 inLength);
		static void			memclr(void* inDest, u32 inLength)									{ memset(inDest, 0, inLength); }
		static void			memcpy(void* inDest, const void* inSrc, u32 inLength);
		static s32			memcmp(const void* inLHS, const void* inRHS, u32 inLength);
		static void			memswap(void* inLHS, const void* inRHS, u32 inLength);
	...
	inline void			x_memset(void* inDest, u32 inValue, u32 inLength)						{ xmem_utils::memset(inDest, inValue, inLength); }
	inline void			x_memclr(void* inDest, u32 inLength)									{ xmem_utils::memclr(inDest, inLength); }
	inline void			x_memcpy(void* inDest, const void* inSource, u32 inLength)				{ xmem_utils::memcpy(inDest, inSource, inLength); }
	inline s32			x_memcmp(const void* inLHS, const void* inRHS, u32 inLength)			{ return xmem_utils::memcmp(inLHS, inRHS, inLength); }
	inline void			x_memswap(void* inLHS, const void* inRHS, u32 inLength)					{ xmem_utils::memswap(inLHS, inRHS, inLength); }

	inline s32			x_memcmp2(const u16* inLHS, const u16* inRHS, u32 inLength)				{ for (u32 i=0; i<inLength; i++, inLHS++, inRHS++) { if (*inLHS < *inRHS) return -1; else if (*inLHS > *inRHS) return 1; } return 0; }
	inline s32			x_memcmp4(const u32* inLHS, const u32* inRHS, u32 inLength)				{ for (u32 i=0; i<inLength; i++, inLHS++, inRHS++) { if (*inLHS < *inRHS) return -1; else if (*inLHS > *inRHS) return 1; } return 0; }
	inline s32			x_memcmp8(const u64* inLHS, const u64* inRHS, u32 inLength)				{ for (u32 i=0; i<inLength; i++, inLHS++, inRHS++) { if (*inLHS < *inRHS) return -1; else if (*inLHS > *inRHS) return 1; } return 0; }

	inline void			x_memzero(void* inBlock, s32 inLength)									{ x_memset(inBlock, 0, inLength); }
	inline void			x_memcopy2(u16* inDest, const u16* inSource, u32 inLength)				{ for (u32 i=0; i<inLength; i++) *inDest++ = *inSource++; } ///< Copy <inLength> aligned 16-bit integers from <inSource> to <inDest>
	inline void			x_memcopy4(u32* inDest, const u32* inSource, u32 inLength)				{ for (u32 i=0; i<inLength; i++) *inDest++ = *inSource++; } ///< Copy <inLength> aligned 32-bit integers from <inSource> to <inDest>
	inline void			x_memcopy8(u64* inDest, const u64* inSource, u32 inLength)				{ for (u32 i=0; i<inLength; i++) *inDest++ = *inSource++; } ///< Copy <inLength> aligned 64-bit integers from <inSource> to <inDest>
	inline void			x_memcopy16(u128* inDest, const u128* inSource, u32 inLength)			{ for (u32 i=0; i<inLength; i++) *inDest++ = *inSource++; } ///< Copy <inLength> aligned 128-bit integers from <inSource> to <inDest>

	inline void*		x_memmove(void* inDest, const void* inSource, u32 inLength)				{ return xmem_utils::memmove(inDest, inSource, inLength); }
	inline void			x_memcopy(void* inDest, const void* inSource, u32 inLength)				{ x_memcpy(inDest, inSource, inLength); }
	inline s32			x_memcompare(const void* inBlock1, const void* inBlock2, u32 inLength)	{ return x_memcmp(inBlock1, inBlock2, inLength); } ///< Return the result of the lexicographical compare between memory block <inBlock1> and <inBlock2> of length <inLength>, return <0 if inBlock1<inBlock2, >0 if inBlock1>inBlock2 and 0 if inBlock1==inBlock2
