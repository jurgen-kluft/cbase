...
namespace xcore
{
	...
	struct xrbnode
	{
		...
		inline void			set_parent(xrbnode* node)					{ u32 flags = (u32)parent & 0x00000003; parent = (xrbnode*)((u32)node | flags); }
		inline xrbnode*		get_parent() const							{ return (xrbnode*)((u32)parent & 0xfffffffc); }

		inline void			set_parent_side(s32 side)					{ ASSERT(side==LEFT || side==RIGHT); if (side==RIGHT) { parent = (xrbnode*)((u32)parent | 0x00000001); } else { parent = (xrbnode*)((u32)parent & 0xfffffffe); } }
		inline s32			get_parent_side() const						{ s32 side = (s32)parent & 0x00000001; ASSERT(side==LEFT || side==RIGHT); return side; }

		inline void			set_red()									{ parent = (xrbnode*)((u32)parent | RED); }
		inline void			set_black()									{ parent = (xrbnode*)((u32)parent & 0xfffffffd); }
		inline void			set_color(s32 colr)							{ if (colr==RED) { parent = (xrbnode*)((u32)parent | RED); } else { parent = (xrbnode*)((u32)parent & 0xfffffffd); } }
		inline s32			get_color() const							{ s32 colr = (s32)parent & 0x00000002; ASSERT(colr == RED || colr == BLACK); return colr; }

		inline bool			is_red() const								{ return ((u32)parent & 0x00000002) == RED; }
		inline bool			is_black() const							{ return ((u32)parent & 0x00000002) == BLACK; }
