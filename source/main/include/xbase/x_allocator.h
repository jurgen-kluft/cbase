...
#define XCORE_CLASS_ARRAY_NEW_DELETE(get_allocator_func, align)       \
    ...
    public:
        inline allocinplace_t(xbyte *data, u64 size)
            : m_base(data), m_data(data), m_size(size)
        {
        }
    ...
    public:
        ...
        template <class T>
        inline T *object() { return (T *)(m_memory); }
    };
    ...
    public:
        dexed_array_t()
            : m_data(nullptr), m_sizeof(0), m_countof(0)
        {
        }
    ...
    // framed allocator
    // allocation is just a forward allocator
    // deallocation does nothing
    // when you end a frame all memory in that frame is released
    class fralloc_t : public alloc_t
    {
    public:
        typedef u32 frame_id_t;
        virtual frame_id_t frame_begin(const char *name) = 0;
        virtual void frame_end(frame_id_t id) = 0;
