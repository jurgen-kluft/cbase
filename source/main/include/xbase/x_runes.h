...
namespace xcore
{
    ...
    namespace utf8
    {
        ...
        void replace(runes& str, rune find, rune replacement);
        ...
        bool        is_lower(crunes const& str);       /// Check if string is all lower case
        bool        is_capitalized(crunes const& str); /// Check if string is capitalized ("Shanghai")
        bool        is_delimited(crunes const& str, rune delimit_left = '\"',
                                 rune delimit_right = '\"'); ///< Check if string is enclosed between left and right delimiter
        inline bool is_quoted(crunes const& str, rune quote = '\"') { return is_delimited(str, quote, quote); }
    ...
    namespace utf32
    {
        ...
        runes find(runes const& _str, uchar32 _c, ECmpMode mode = CASE_SENSITIVE);
        runes find(runes const& str, crunes const& find,
                   ECmpMode mode = CASE_SENSITIVE); // Return position of first occurrence of <inString> or -1 if not found
        runes find_one_of(runes const& str, crunes const& set,
                          ECmpMode mode = CASE_SENSITIVE); // Return position of first occurrence of a character in <inCharSet>
                                                           // after <inPos> or -1 if not found

        crunes find(crunes const& _str, uchar32 _c, ECmpMode mode = CASE_SENSITIVE);
        crunes find(crunes const& str, crunes const& find,
                    ECmpMode mode = CASE_SENSITIVE); // Return position of first occurrence of <inString> or -1 if not found
        crunes find_one_of(crunes const& str, crunes const& set,
                           ECmpMode mode = CASE_SENSITIVE); // Return position of first occurrence of a character in <inCharSet>
                                                            // after <inPos> or -1 if not found
