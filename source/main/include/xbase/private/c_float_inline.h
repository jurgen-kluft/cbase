...
enum EFloatIEEE
{
    FF_FRACTION_MASK            = 0x007FFFFF,    FF_GET_FRACTION_SHIFT    = 0,    FF_FRACTION_BITS = 23,
    FF_EXPONENT_MASK            = 0x7F800000,    FF_GET_EXPONENT_SHIFT    = 23,    FF_EXPONENT_BITS = 8,
    FF_SIGN_MASK                = 0x80000000,    FF_GET_SIGN_SHIFT        = 31,    FF_SIGN_BITS = 1,
...
namespace xf32
{
    ...
    /**
     * @return  This funtion returns the smaller one of the two numbers.
     */
    inline f32      fmin(f32 numberA, f32 numberB)            { return numberA <= numberB ? numberA : numberB; }

    /**
     * @return  This funtion returns the larger one of the two numbers.
     */
    inline f32      fmax(f32 numberA, f32 numberB)            { return numberA >= numberB ? numberA : numberB; }

    /**
     * @return  This funtion returns the value clamped between Low and High.
     */
     ...
     */
    inline f32        positiveZero()                        { u32 f = (u32)0; return toF32(f); }
    inline f32        negativeZero()                        { u32 f = (u32)FF_SIGN_MASK; return toF32(f); }
#ifdef IEEE_FLOATS
    inline f32        nan()                                 { u32 f = (u32)(FF_EXPONENT_MASK | 1); return toF32(f); }
    inline f32        positiveInfinity()                    { u32 f = (u32)(FF_EXPONENT_MASK); return toF32(f); }
    inline f32        negativeInfinity()                    { u32 f = (u32)(FF_SIGN_MASK | FF_EXPONENT_MASK); return toF32(f); }
#endif
    inline f32        positiveMaximum()                     { u32 f = (u32)((((1<<FF_EXPONENT_BITS)-2)<<FF_GET_EXPONENT_SHIFT) | FF_FRACTION_MASK); return toF32(f); }
    inline f32        negativeMaximum()                     { u32 f = (u32)((FF_SIGN_MASK | (((1<<FF_EXPONENT_BITS)-2)<<FF_GET_EXPONENT_SHIFT)) | FF_FRACTION_MASK); return toF32(f); }

    /**
     * De-normalized (DEN) positive and negative minimums.
     */
    inline f32        positiveMinimum()                     { u32 f = (u32)(1<<(FF_FRACTION_BITS-1)); return toF32(f); }
    inline f32        negativeMinimum()                     { u32 f = (u32)(FF_SIGN_MASK | (1<<(FF_FRACTION_BITS-1))); return toF32(f); }
     ...
     */
    inline s32      sign(f32 inNumber)                      { if (bin(inNumber) & ~FF_SIGN_MASK) return 1 | signBitSigned(inNumber); else return 0; }
     ...
     */
    inline s32      sign(f32 inNumber, f32 inDelta)         { ASSERT(inDelta >= 0.0f); return abs(inNumber) <= inDelta ? 0 : sign(inNumber); }
