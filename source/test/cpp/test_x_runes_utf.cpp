...
UNITTEST_SUITE_BEGIN(xstring_utf)
{
    UNITTEST_FIXTURE(utf8)
    {
        ...
        {
            crunes_t src("test");
            CHECK_EQUAL('t', src.m_runes.m_ascii.m_str[0]);
            CHECK_EQUAL('e', src.m_runes.m_ascii.m_str[1]);

            runez_t<utf32::rune, 16> dst;
            copy(src, dst);

            CHECK_EQUAL('t', dst.m_runes.m_utf32.m_str[0]);
        }

        UNITTEST_TEST(read1_utf8)
        {
            crunes_t  str1((utf8::pcrune) "test", nullptr);
            crunes_t str11((ascii::pcrune) "test", nullptr);
			runes_reader_t reader1(str1);
            for (s32 i = 0; i < 5; ++i)
            {
                uchar32 ch = reader1.read();
                CHECK_EQUAL((uchar32)str11.m_runes.m_utf8.m_str[i], ch);
            }
